...
//3 types of procedural macros: Function-Like, Attribute-Like, Custom-Like
#[derive(FromMeta)]
struct MacroArgs{
    #[darling(default)]
    verbose: bool,
...
fn impl_log_call(attr_args: &MacroArgs, input: &mut ItemFn) -> TokenStream{   
    ...
    if attr_args.verbose {
        ...
        let statments = generate_verbose_log(fn_name, fn_args);

        //3. Prepend verbose log message to function block
        input.block.stmts.splice(0..0, statments);
    } else {
...
fn extract_arg_names(func: &ItemFn) -> Vec<&Ident> {    
    func.sig.inputs.iter().filter_map(|arg: &FnArg| {
...
pub fn log_derive(input: TokenStream) -> TokenStream{
    let ast: syn::DeriveInput = syn::parse(input).unwrap(); //parse to syntex tree
    let name = &ast.ident;
